...
jest.unstable_mockModule("../src/services/authService.js", () => ({
  sendVerificationEmail: jest
    .fn()
    .mockResolvedValue({ success: true, message: "Verification email sent!" }),
}));
...
beforeAll(async () => {
  // Import the server and authService modules
  const { default: importedServer } = await import("../server.js");
  ...
  app = server; // If needed, assign app for convenience
});
...
describe("Auth Controller Integration Tests", () => {
  ...
  it("should return 400 if reset-password params are missing", async () => {
    ...
    expect(res.body).toHaveProperty(
      "error",
      "oobCode and new password are required."
    );
  });
});
