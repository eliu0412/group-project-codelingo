...
let app, server, mockGet, mockPush, mockRef, mockQuery, mockOrderByChild, mockEqualTo;
beforeAll(async () => {
  const serverModule = await import('../server.js'); // Adjust path if server.js is elsewhere
...
describe('Lesson Controller Integration Tests', () => {
  ...
  it('should return 400 if topic or description is missing', async () => {
    const res = await request(app)
      ...
      .send({ problemDifficulty: 'easy', problemType: 'math', problemTags: ['algebra'] });
    expect(res.statusCode).toBe(400);
    expect(res.body).toHaveProperty('error', 'Fields (topic, description) are required.');
  });
  ...
  it('should return 404 if no problems found for the specified parameters', async () => {
    ...
    expect(res.body).toHaveProperty('error', 'No problems found for the specified parameters.');
  });
  ...
  it('should return 404 if no problems match the type and tags', async () => {
    mockGet.mockResolvedValue({
      exists: () => true,
      forEach: (callback) => {
        callback({ val: () => ({ problemType: 'science', tags: ['biology'] }) });
        return true;
      },
    });
    mockRef.mockReturnValue({ get: mockGet });
